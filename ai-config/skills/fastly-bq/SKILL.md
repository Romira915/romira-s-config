---
name: fastly-bq
description: Fastly access logs の BigQuery テーブル (`pr-times-big-query.fastly_access_logs.prtimesprod-fastly-log`) のテーブル名・カラム定義・サンプル SQL を即座に提示する。Fastly ログを SQL で調査したいときに使う。
allowed-tools: Read
---
...
- **プロジェクト**: `pr-times-big-query`
- **データセット**: `fastly_access_logs`
- **テーブル**: `prtimesprod-fastly-log`
- **FQN**: `` `pr-times-big-query.fastly_access_logs.prtimesprod-fastly-log` ``
...
1. **まずこのスキルの情報をそのまま提示**: テーブル名・必要なカラム・パーティション条件を簡潔に示す
2. **ユーザー要望に合わせたサンプル SQL を提示**: 以下のテンプレートから用途に合ったものを選んで埋める
3. **フルスキャン防止**: 必ず `timestamp` で期間を絞る SQL にする
4. **タイムゾーンは JST 固定**: 条件指定も SELECT での表示も `Asia/Tokyo` を使う
5. **LIMIT**: 調査用クエリは `LIMIT 1000` など上限を付ける
...
SELECT
  ...
  client_ip,
  request_user_agent
FROM `pr-times-big-query.fastly_access_logs.prtimesprod-fastly-log`
