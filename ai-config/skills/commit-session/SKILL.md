...
今回のセッションで修正したファイルだけを commit / push する。作業開始時の `git status --short` を記録し、開始時点から存在する変更や今回の作業と無関係な変更は対象外にする。ただし、依頼文で対象として明示された既存変更・未追跡ファイルは対象候補として記録し、検証後に含める。`git add -A` や `git add .` は使わず、対象ファイルを明示して `git add` する。

未追跡ファイルは、未追跡であることだけを理由に除外しない。ユーザーが新規スキルやファイルを今回の成果物として指定している場合は、内容を検証したうえで明示的にステージする。依頼文で指定されていない未追跡ファイルは対象外にして残す。
