...
新規作成・既存スキルの編集を問わず、書き込む前に内容を次の層へ分類する。

- `SKILL.md`: スキルが発火するたびに必要な中核手順、判断基準、安全上の必須条件、追加ファイルを読む条件
- `references/`: 特定シナリオだけで使う詳細、製品・プロトコル固有の知識、長い例、トラブルシューティング
- `scripts/`: 毎回同じ手順で行う決定的な処理。本文へ長いコードを複製せず、実行方法と入出力だけを `SKILL.md` に書く
- `assets/`: 出力へ利用するテンプレートや素材

`SKILL.md` を目次兼実行ガイドとして簡潔に保ち、条件付きの詳細を本文へ積み上げない。reference を追加するときは、`SKILL.md` に「どの条件で」「何のために」読むかを明記する。逆に、全実行で必要な安全条件や中核判断を reference だけへ隠さない。

一つの案件・サービス・配信基盤だけに当てはまる知識は、スキル全体の中核ルールへ一般化しない。複数案件で再利用できるなら匿名化した reference に分離し、案件固有の値や一回限りの事実ならプロジェクト文書または実行結果へ記録する。

特定タスクでユーザーが一度だけ許可したコマンドや外部サービス操作は、スキルが通常必要とする依存関係ではない。今回使用したという理由だけで `allowed-tools` へ追加せず、今後も同じスキルの標準手順として必要かを確認してから最小権限で追加する。
