...
- **ドメイン移行/ホスト追随系は URL バー（アドレスバー）が見えるウインドウスクショで撮る**。ページ本文だけでは `.com` 経由か `.net` 経由か判別できず、URL バーが核心の証跡になる
- **macOS + Playwright MCP（headed Chrome）でのウインドウ撮影方法**:
  - `screencapture -l <CGWindowID>` は Chrome の**コンテンツ層だけで omnibox が写らない**ので使わない
  - `screencapture -R x,y,w,h` は**複数ディスプレイ＋Retina で座標がズレる**ので不安定
  - 確実なのは「**対象ディスプレイ全体を `screencapture -D<n>` で撮り、ウインドウ領域を crop（PIL 等）**」。手順:
    1. AppleScript で**対象 URL を持つ Chrome ウインドウを `set index ... to 1` で前面化**（`activate` だけだと別の Chrome ウインドウが前面に来て誤撮影する）
    2. `bounds of window 1`（global 座標）を取得し、ディスプレイ原点を引いて局所座標へ変換
    3. `screencapture -D<displayindex> -x` で撮影 → ウインドウ領域を crop
