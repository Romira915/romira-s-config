...
- PR description・チケット・画面に記載された正式な機能名、画面名、操作名
...
- **Obsidian Vault 内**（`.obsidian/` が cwd または親に存在）: `Work/開発/<project>/<Jira-no>-<環境>-QA結果.md`
  ...
  - Vault 側に既存の関連プロジェクトフォルダがある場合は、repo 側 `docs/qa/` より Vault 側を優先する運用のことがある（ユーザーの過去の指示があれば従う）
...
2. **セクション見出しは絵文字付き `###`**。順番・文言を固定する:
   ...
   - `### 🌐 影響範囲` — 影響するサービス／バッチ等。リリース前後で挙動や確認条件が変わる場合のみ、その状態を書く
   - `### 🔍 テスト観点` — 確認したい観点を箇条書き（`*`）で
   - `### 📝 確認内容・確認結果` — 下記の表

   **粒度**: 読んだ人がそのまま操作できる具体性にする。カテゴリ・種類・enum・ID が関わる場合は「具体名 + ID + 期待する DB/ログ値」まで書く。曖昧な「該当カテゴリを設定する」だけで終わらせない。STG 画面操作や通常運用で再現できない前提条件（例: LLM の検証エラーを意図的に起こす、外部 API を異常応答にする）は手動 QA 表から外す。必要なら自動テスト・コードレビュー観点として別扱いにする。

   **簡潔さ**: 具体性は情報量ではなく、操作と合否判定の明確さで担保する。各文は「操作」「期待結果」「影響範囲」のいずれかに必要な情報だけを書く。
   - 前後差を説明しない単独の状態ラベル（例: 「（リリース前）」）は書かない
   - 概要・影響範囲・テスト観点で同じ実装説明を言い換えて繰り返さない
   - テーブルで具体化する内容を、前段の各セクションで先回りして詳述しない
   - 機能名・画面名・操作名は PR description・チケット・画面上の正式名称をそのまま使い、実装内容から略称や別名を作らない。正式名称を特定できない場合はユーザーに確認する

3. **確認内容・確認結果は 7 列の表**: `前提条件 | 画面/アクション | テスト対象 | 手順 | 期待結果 | 結果 | エビデンス`
   - 手順セルに複数行のコードブロックを含む場合は、パイプ表ではなく HTML の `<table>` / `<tr>` / `<th>` / `<td>` を使う
   - **結果**列は絵文字。未検証は ⬜️、検証後に ✅ / ❌ に更新する
   - **期待結果**列は合格条件を観測可能な値・状態で書く。条件が複数ある場合は、1条件を1項目として `<ul><li>条件1</li><li>条件2</li></ul>` のように箇条書きにする。複数条件を一段落へ詰め込まない
   - **エビデンス**列に実行した確認コマンドと実出力、またはスクショを入れる。合否だけを言い換えた要約文はエビデンスにしない
   - CLI のエビデンスは、確認コマンドとその出力を同じ `<pre><code class="language-console">` に `$ command`、続けて出力の順で記載する。複数の確認コマンドがある場合は、目的ごとに見出しとコードブロックを分ける
   - 値の一致を根拠にする場合は比較コマンドと比較対象の実値を載せる。値が存在しないことを根拠にする場合は検索条件がわかるコマンドと、`none` または終了ステータスなど不在を判定できる出力を載せる
   - テキスト出力は **一致部分だけを切り出さない** — HTTP ヘッダー、HTML の該当行の前後数行、JSON の関連フィールドなど、**「何にマッチしたか」が読者にわかる文脈を残す**。セル内改行は行末に半角スペース 2 個＋改行（`  \n`）を使う
...
9. **実行するコマンドは同じ確認行の手順セル内にコードブロックとして埋め込む**。表外のコマンド集へ分離しない
   - `<pre><code class="language-sh">` または `<pre><code class="language-sql">` の中に、実際の改行を含むコマンドを書く
   - `&#10;` などの数値文字参照で改行を代用しない。レンダラーによって文字列のまま露出するため使用禁止
   - 変数定義や必要なオプションもコードブロックへ含める。秘密値は直書きせず、対話入力または環境変数で受け取る
   - キャッシュの動作確認では、キャッシュキーを変えるクエリパラメータを追加しない。既存キャッシュの除去が必要なら、対象 URL を `curl -X PURGE` してから同じ URL を取得する
   - 検証前に残っていたキャッシュなど、一時的な環境状態の説明は QA 本文・結果コメントに残さない。実行に必要なコマンドだけを手順に記載する
...
<影響するサービス／バッチ。前後で挙動や確認条件が変わる場合のみ状態を書く>
...
command ...</code></pre>
      ...
      <td><ul><li>合格条件1</li><li>合格条件2</li></ul></td>
      <td>⬜️</td>
      <td></td>
    </tr>
  </tbody>
...
- 自明な説明や重複を削り、削除によって操作・合否判定・影響範囲が変わらない文を残さない
