...
- 画面を伴う QA で、正式名称の確認またはケースの分割・統合判断が必要な場合だけ `references/ui-inspection.md` を Read する
- 取得済みスクリーンショットを QA 結果へ掲載する場合だけ `references/evidence-rendering.md` を Read する
...
- ローカル開発環境で実行する単体テスト、静的解析、lint、フォーマット等の実装検証（STG 上の挙動を観測する QA ではない）
...
STG と PROD の両方に変更がある場合も、両環境の QA を同時に設計・確認しない。リリースと確認は次の順序で進める:

1. STG へリリースする
2. STG QA を実施する
3. STG QA の結果を第三者が確認する
4. OK なら PROD へリリースする
5. PROD QA を実施する
6. PROD QA の結果を第三者が確認する

- QA ドキュメントは現在のリリース段階に対応する環境だけを対象にし、STG 用と PROD 用を別ファイルにする
- STG QA の作成時に PROD の確認項目・確認ホスト・結果欄を含めない
- PROD QA は、STG QA と第三者確認が完了して PROD リリースへ進む段階で作成する
- ユーザーが現在の段階を明示していない場合は、リリースフローの先頭である STG QA として扱う。PROD も同時に確認する前提では質問しない
- 第三者確認はリリース判断の工程であり、手動 QA 表のテストケースとしては追加しない
...
`<環境>` は PR description、差分、現在のリリース段階から STG/PROD 等を決める。両環境に変更があってもファイル名を `STG-PROD` のようにまとめず、現在の段階に対応する1環境だけを使う。段階が不明なら STG をデフォルトにする。
...
3. **確認内容・確認結果は 7 列の表**: `前提条件 | 画面/アクション | テスト対象 | 手順 | 期待結果 | 結果 | エビデンス`
   ...
   - 未検証のチェックリストでは、スクショで示す項目のエビデンス列を空欄にする（「ここに画像を貼付」等のプレースホルダ文言は書かない）
   - 検証後にスクリーンショットを取得済みなら、ファイル名だけを並べず `references/evidence-rendering.md` に従ってエビデンスセル内へ画像を表示する
   - **ドメイン移行/ホスト追随系のブラウザ項目は URL バー（アドレスバー）が見えるスクショが前提**（撮り方は手順 6 を参照）
